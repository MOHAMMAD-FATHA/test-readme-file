                ...
                if validate_config(cfg):
                    ...
                    for station_id, station_cfg in CONFIG.get("stations", {}).items():
                        ...
                        if old_status == "active" and new_status == "paused":
                            ...
                            # if is_break_time_for_station(station_id):
...
def convert_shift_name(template_name: str) -> str:
    """
    Convert shift template name to output format.
    Example:
      Shift2 -> 2shifts
      Shift3 -> 3shifts
    """
...
def flatten_zone_config_to_station_config(input_data: dict) -> dict:
    # desired = input_data["state"]["desired"]
    ...
    for template_name, template_shifts in shift_templates.items():
        ...
        for shift_name, shift_data in template_shifts.items():
            # Shift2-A -> A, Shift3-B -> B
            short_name = shift_name.split("-")[-1]
...
def save_shadow_config(cfg):
    try:
        ...
        safe_print("[SHADOW] Local config written from shadow")
    except Exception as e:
        safe_print("[SHADOW] Failed writing config:", e)

# -------------------------
# SHADOW MQTT CALLBACKS
# -------------------------
def shadow_on_connect(client, userdata, flags, rc, properties=None):
    if rc == 0:
        safe_print("[SHADOW] Connected")
        ...
        safe_print("[SHADOW] Initial GET requested", SHADOW_GET_ACCEPTED)
    else:
        safe_print("[SHADOW] Connect failed rc=", rc)


def shadow_on_message(client, userdata, msg):
    global GET_IN_FLIGHT   # MUST be first
    try:
        data = json.loads(msg.payload.decode())
    except Exception:
        safe_print("[SHADOW] Invalid JSON payload")
        return

    safe_print(f"[SHADOW] Message on {msg.topic}")

    # ------------------------------------------------------
    # WRITE CONFIG — ONLY HERE
    # ------------------------------------------------------
    if msg.topic == SHADOW_GET_ACCEPTED:
        ...
        if not desired:
            safe_print("[SHADOW] No desired config")
            return
    ...
    if msg.topic in (SHADOW_UPDATE_DELTA, SHADOW_UPDATE_ACCEPTED):
        # safe_print("[SHADOW] Delta/update detected → requesting full GET")
        # client.publish(SHADOW_GET_TOPIC, "{}")
        if not GET_IN_FLIGHT:
            print("[SHADOW] Delta/update detected → requesting full GET")
            GET_IN_FLIGHT = True
            client.publish(SHADOW_GET_TOPIC, "{}")
        else:
            print("GET already in flight, skipping")
        return

# -------------------------
# SHADOW CLIENT THREAD
# -------------------------
def shadow_client_thread():
    if not AWS_THING_NAME or not AWS_SHADOW_NAME or not AWS_ENDPOINT:
        safe_print("[SHADOW] Shadow not configured → skipping")
        return

    safe_print("[SHADOW] Starting shadow client")
    ...
    client.on_connect = shadow_on_connect
    client.on_message = shadow_on_message

    try:
        client.connect(AWS_ENDPOINT, AWS_PORT, keepalive=60)
        client.loop_start()
    except Exception as e:
        safe_print("[SHADOW] Connection failed:", e)
        return
    ...
    client.loop_stop()
    client.disconnect()
    safe_print("[SHADOW] Client stopped")
...
def get_shift_for_station(station_id):
    ...
    # print("\n[SHIFT-DEBUG] ===============================")
    # print(f"[SHIFT-DEBUG] Station: {station_id}")
    # print(f"[SHIFT-DEBUG] Current UTC time: {now_dt}")
    ...
    try:
        ...
        for sname, win in group.items():
            ...
            if end_t > start_t:
                # start_dt = datetime.combine(now_dt.date(), start_t, tzinfo=timezone.utc)
                # end_dt   = datetime.combine(now_dt.date(), end_t, tzinfo=timezone.utc)
                ...
                # print("[SHIFT-DEBUG] Normal shift")
            ...
            # OVERNIGHT SHIFT (crosses midnight, e.g., 18:00 → 06:00)
                ...
                if now_time < end_t:
                    # start_dt = datetime.combine(now_dt.date() - timedelta(days=1), start_t, tzinfo=timezone.utc)
                    # end_dt   = datetime.combine(now_dt.date(), end_t, tzinfo=timezone.utc)
                    ...
                    # print("[SHIFT-DEBUG] now < end_t → shift started YESTERDAY")
                else:
                    # current time ≥ start → before midnight → shift starts today
                    # start_dt = datetime.combine(now_dt.date(), start_t, tzinfo=timezone.utc)
                    # end_dt   = datetime.combine(now_dt.date() + timedelta(days=1), end_t, tzinfo=timezone.utc)
                    # print("[SHIFT-DEBUG] now ≥ start_t → shift starts TODAY")
...
def is_break_time_for_station(station_id):
    ...
    for br in break_list:
        ...
        if not start_t or not end_t:
            continue
        # s = datetime.combine(now.date(), start_t, tzinfo=timezone.utc)
        # e = datetime.combine(now.date(), end_t, tzinfo=timezone.utc)
...
def publish_machine_state(station_id, state, by='main'):
    ...
    try:
        enqueue_publish(PUB_TOPIC, payload)
        # save_runtime_state(station_id, "machine_state","current",
        #             {
        #                 "state": state,
        #                 "ts": time.time(),
        #                 "by": by
...
def worker_watchdog_fn():
    ...
    while not shutdown_event.is_set():
        ...
        if not wt.is_alive():
            ...
            t.start()
    #     else:
    #         # check last processed timestamp to see if stalled
    #         if time.time() - last_worker_processed_ts > WORKER_STALL_TIMEOUT:
    #             safe_print("[WATCHDOG] worker appears stalled; attempting to restart")
    #             try:
    #                 # best-effort: start new worker thread (old one may be hung)
    #                 t = Thread(target=worker_fn, daemon=True)
    #                 with threads_lock:
    #                     worker_thread = t
    #                 t.start()
    #             except Exception as e:
    #                 safe_print("[WATCHDOG] failed to restart worker:", e)
            ...
            for sid in list(station_ids_plan_downtime):
                ...
                if current != previous:
                    ...
                    if current:
                        ...
                        new_state = determine_machine_state(sid, include_planned=False)
                        publish_machine_state(sid, new_state, by="break")
                    # new_state = determine_machine_state(sid, include_planned=True)
                    # publish_machine_state(sid, new_state, by="break")
...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        #     safe_print("[SHADOW] Subscribed:", SHADOW_DELTA_TOPIC)
        # except Exception:
        #     pass
        mqtt_connected = True
        try:
...
def compute_daily_and_publish():
    ...
    while not shutdown_event.is_set():
        ...
        # if sleep_seconds > 0:
        #     shutdown_event.wait(timeout=sleep_seconds + 1)
        # if shutdown_event.is_set():
        #     break
        # day = (next_midnight - timedelta(days=1)).strftime('%Y-%m-%d')
        # safe_print(f"[DAILY] computing for {day}")
        ...
        try:
            with db_lock:
                ...
                for (station_id, operator_sec, diesetter_sec, downtime_sec, planned_run_mins, good_parts, scrap_parts,
                    ...
                    # OA_daily = f"{float(avg_OA)}:{shift}"
                    # QR_daily = f"{float(avg_QR)}:{shift}"
                    ...
                    # payload_oa_daily = json.dumps({
                    #     "propertyAlias": f"{SITEWISE_MODEL_NAME}/{station_id}/PerformancePerJob",
                    #     "propertyValues": [{
                    #         "timestamp": {"timeInSeconds": int(time.time())},
                    #         "quality": "GOOD",
                    #         "value": {"stringValue": OA_daily}
                    #     }]
                    # })

                    # payload_qr_daily = json.dumps({
                    #     "propertyAlias": f"{SITEWISE_MODEL_NAME}/{station_id}/QualityPerJob",
                    #     "propertyValues": [{
                    #         "timestamp": {"timeInSeconds": int(time.time())},
                    #         "quality": "GOOD",
                    #         "value": {"stringValue": QR_daily}
                    #     }]
                    # })

                    # enqueue_publish(PUB_TOPIC, payload_oa_daily)
                    # enqueue_publish(PUB_TOPIC, payload_qr_daily)

                    # safe_print(f"[PUBLISH-DAILY] {station_id} OA={OA_daily} QR={QR_daily}")
...
def main():
    ...
    Thread(target=shadow_client_thread, daemon=True).start()
